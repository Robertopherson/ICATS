...
choice. It first estimates the density of states `Omega(J)` generated by
independent `L` and `Jab` sampling, then uses an acceptance weight of the form
...
to bring the accepted ensemble closer to the desired total-`J` measure.
