        ...
        while sum(abs(mox)) <= 0.0:
            for xi in range(len(xx)):
                if random.random() > 0.60:
                    mox[xi] += 1
                    nx[xi] += (random.random() - 0.5) * dl[xi]
        nx = putwithindomain(nx, xx, dl, domain)
        ne = Fx(nx, *par)
        if debug:
            print('pnx = ', nx, (random.random() - 0.5) * 2 * dl[xi], 'PAR = ', par, 'NE = ', ne)
        k += 1
    if k >= 10000:
        print(oe, xx, ne, nx)
    ...
    if not domain is None:
        for xi in range(len(nx)):
            if nx[xi] < domain[xi][0]:
                if domain[xi][2]:
                    nx[xi] = domain[xi][1] + ( nx[xi] - domain[xi][0] )
                    #nx[xi] = domain[xi][1] + nx[xi]
                else:
                    nx[xi] = random.random() * xx[xi]
            elif nx[xi] > domain[xi][1]:
                if domain[xi][2]:
                    nx[xi] = domain[xi][0] + (nx[xi] - domain[xi][1])
                else:
                    nx[xi] = xx[xi] + random.random() * (domain[xi][1] - xx[xi])
    return nx
...
def MCstep(xx, Fx, par, dl, *oes, **dic):
    ...
    if ne / (oe + 1.0e-15) > 1.0:
        ...
        if ne / (oe + 1.0e-15) > random.random():
            pas = True
        else:
            pas = False
            ne = oe
...
def EstimateDl(x0, pfr, Fx, par, idl, **dic):
    ...
    xx = x0
    dim = len(x0)
    dl = array([idl for i in range(dim)])
...
def InitMC(x0, Fx, par, **dic):
    """
    Initiate Sampling using a Metropolis-Hastings Markov Chain Monte Carlo.
    ...
    if 'idel' in dic.keys():
        ...
        idel = 1.0
    nsamp = 10000
    # restart parameters: par, domain, last point, last energy, delta step dl, ps, fl 
    xx = copy(x0)
    dim = len(x0)
    dl = EstimateDl(xx, 0.75, Fx, par, idel, domains=domain)
    if debug:
        print('DL = ', dl)
    ndecor = EstimateCorr(xx, 0.75, Fx, par, dl, domains=domain)
    if debug:
        print('NDECOR = ', ndecor)
    ...
    xx, oe, mox = step(xx, oe, Fx, par, domain, dl)
    it = 0 
    cont = [xx, oe, it, Fx, par, dl,  domain, ps, fl]
    if 'niter' in dic.keys():
      niter = dic['niter']
      out = [ MCsample(cont)[:2] for i in range(niter) ]
      xout, eout = [o[0] for o in out], [o[1] for o in out]
      if debug:
          ps, fl = cont[-2:]
          print('PAS = ', ps, 'FAIL = ', fl)
      return cont, xout, eout 
    else:
      return cont 
...
def MCsample(cont):
    ...
    Parameters:
    cont (list): conains previous [xx, ee, function, parameters, dl, domain, ps, fl] 

    Returns:
    list: updated cont list  
    """
    xx, oe, it, Fx, par, dl, domain, ps, fl =  cont 
    xxx, ooe, pas, mox = MCstep(xx, Fx, par, dl, oe, domains=domain)  
    cont[0], cont[1] = xxx.copy(), ooe 
    cont[2] += 1
    if pas:
        cont[-2] += mox
    else:
...
def InitICDF(dim, Fx, par, **dic):
    """
    Initiate Sampling using the inverse of the CDF .

    Parameters:
    Fx (function): iCDF to evaluate the energy of a state.
    par (tuple): Parameters required by the energy function Fx.
    dic (dict): Additional parameters, including 'domains' for domain constraints and 'idel' for initial step size.
    ...
    debug = False
    if 'seed' in dic.keys():
      seed = [dic['seed']*(i+1) for i in range(dim)]
    else:
      seed = [12345*i for i in range(dim)]
    rng = [np.random.default_rng(seed[i]) for i in range(dim) ]
    state = [rng[i].bit_generator.state for i in range(dim)]
    if 'par0' in dic.keys():
      xx, oe = Fx(rng, *par,*dic['par0'])
    else:
      xx, oe = Fx(rng, *par)
    it = 0 
    cont = [xx, oe, it, state, seed, Fx, par ]
    if 'niter' in dic.keys():
      niter = dic['niter']
      out = [ ICSFsample(cont)[:2] for i in range(niter) ]
      xout, eout = [o[0] for o in out], [o[1] for o in out]
      return cont, xout, eout 
    else:
      return  cont 

def ICDFsample(cont,*lis):
    """
    Sample states using a uniform distribuition  and a ICDF.

    Parameters:
    cont (list): conains previous [xx, oe, it, seed, state, Fx, par ] 

    Returns:
    list: updated cont list  
"""
    xx, oe, it, state, seed, Fx, par =  cont
    dim = len(seed)

    rng = [np.random.default_rng(seed[i]) for i in range(dim) ]
    u = []
    for i in range(dim): 
      rng[i].bit_generator.state = state[i]
    if len(lis) > 0:
      xx, oe = Fx(rng,*par,*lis)
    else:
      xx, oe = Fx(rng,*par)
    cont[0], cont[1] = xx.copy(), oe 
    cont[2] += 1
